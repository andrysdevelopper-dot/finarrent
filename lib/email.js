...
<body>
  <h2 style="color:#0A2540;">Finassur</h2>
  <p>Bonjour,</p>
  ...
  <p>Cordialement,<br>L'équipe Finassur</p>
</body>
</html>`;
  ...
  try {
    await trans.sendMail({
      from,
      to,
      subject: `Demande ${reference} enregistrée - Finassur`,
      html,
    });
    return { sent: true };
  } catch (err) {
...
<head><meta charset="utf-8"><style>body{font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#333;line-height:1.6}</style></head>
<body>
  <h2 style="color:#0A2540;">Nouvelle demande Finassur</h2>
  <p><strong>Référence :</strong> ${reference}</p>
  <p><strong>Entreprise :</strong> ${companyName}</p>
  ...
  try {
    await trans.sendMail({
      from,
      to: adminEmail,
      subject: `[Finassur] Nouvelle demande ${reference} - ${companyName}`,
      html,
    });
    return { sent: true };
  } catch (err) {
